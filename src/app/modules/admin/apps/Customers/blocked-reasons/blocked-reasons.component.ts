...
import { ReasonModel } from '../CustomerModels/reason';
...
export class BlockedReasonsComponent implements OnInit {
  ...
  AddForm!: FormGroup;
  EditForm!:FormGroup;
  constructor(private router: Router,public formBuilder: FormBuilder,private messageService: MessageService,private confirmationService: ConfirmationService,private customerservice:CustomerService) { }
  ...
  AddReason(){
    ...
    this.customerservice.addReason(this.product.name).subscribe({
      next: (response)=>{
...
this.showEdit=true;
  ...
  Delete(data){
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.customerservice.deleteblockedReason(data.id).subscribe(
          {
            next: (response)=>{
              this.GetAllBlockReason();  
        },
  ...
  EditedReason(){
    ...
    this.customerservice.editblockedReason(product).subscribe(
      {
        next: (response)=>{
          this.GetAllBlockReason(); 
          this.CancelEdit(); 
